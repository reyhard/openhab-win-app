...
| Device-state mapping | partial | Foundation status says Task 7 added Windows device state telemetry mapping from battery, charging, lock, and session snapshots to configured openHAB Item state updates; `rg --files` shows `DeviceStateMapper.cs` and `DeviceStateMapperTests.cs`. Current settings evidence shows `AppSettings.cs` has no Item-name mapping fields, and the audit did not find settings UI for configuring the target openHAB Items or a Windows collector/sender path. | Treat the mapper as shipped foundation code only. Track configurable target Items, real Windows state collection, scheduled/event-triggered sends, current Wi-Fi/network status, and openHAB connection-status telemetry as remaining work. |
...
### D4: Device-state telemetry is foundation-only, not user-configurable shipped behavior

- **Severity:** Low
- **Priority:** Documentation/Tracking
- **Evidence:** `src\OpenHab.Core\DeviceState\DeviceStateMapping.cs` and `DeviceStateMapper.cs` define mapper inputs for battery, charging, locked, and session state, and `tests\OpenHab.Core.Tests\DeviceStateMapperTests.cs` covers pure mapping behavior. `src\OpenHab.App\Settings\AppSettings.cs` does not define target openHAB Item names for device telemetry, and the current settings UI files do not expose configuration for battery, charging, lock, session, Wi-Fi, or connection-status Items.
- **Impact:** Calling device-state mapping "shipped" overstates the user-visible feature. The shipped part is a core mapper, while the app still lacks Item configuration, Windows state collection, send scheduling, and additional telemetry such as current Wi-Fi/network status.
- **Suggested direction:** In the consolidated tracker, classify device-state telemetry as partial: mapper shipped, configuration and sending pending. Add a dedicated backlog item for configurable device and connectivity telemetry.
- **Affected files/layers:** `src\OpenHab.Core\DeviceState\*.cs`, `src\OpenHab.App\Settings\AppSettings.cs`, Windows settings UI under `src\OpenHab.Windows.Tray`, and the future consolidated tracker.
- **Verification needed:** Confirm the tracker distinguishes core mapping from complete telemetry behavior, and add tests when settings fields, collectors, and sender orchestration are implemented.
...
### B8: Add configurable device and connectivity telemetry

- **Priority:** P1
- **Problem:** The core device-state mapper exists, but users cannot configure which openHAB Items receive battery, charging, locked, session, Wi-Fi, or connection-status values, and the app does not yet collect and send those values as a complete feature.
- **Evidence:** `D4` cites `DeviceStateMapping`, `DeviceStateMapper`, mapper tests, and the absence of telemetry Item configuration in `AppSettings`. The original sitemap design names `PcBatteryLevel`, `PcChargingState`, `PcLockedState`, and `PcSessionState` as expected telemetry targets; user feedback adds current Wi-Fi/network and connection-status telemetry as needed features.
- **Suggested change:** Extend settings with optional target Item names for battery level, charging state, locked state, session state, Wi-Fi/network status, and connection status; add Windows collectors for the values that are not already available; add a sender/orchestrator that posts mapped states through the active openHAB client on startup, relevant system events, and a conservative interval.
- **Affected files/layers:** `src\OpenHab.Core\DeviceState\*.cs`, `src\OpenHab.App\Settings\AppSettings.cs`, `src\OpenHab.App\Settings\AppSettingsController.cs`, a new app/runtime telemetry sender, Windows-specific collectors under `src\OpenHab.Windows.Tray` or a future Windows device-state layer, and settings UI in `MainWindow.xaml` / `MainWindow.xaml.cs`.
- **Verification:** Add settings serialization/migration tests, mapper tests for new Wi-Fi/connection fields, collector tests where feasible through abstractions, sender tests that verify only configured Items are sent and telemetry failures do not affect sitemap browsing, and manual settings UI checks for configuring and clearing Item names.
- **Risk/notes:** Keep telemetry optional and non-blocking. Avoid sending sensitive network identifiers by default; define exactly what Wi-Fi/network status means before exposing it as an openHAB state.
...
- `rg -n "DeviceState|Battery|Charging|Locked|Session|Wi-?Fi|Connection|Network|Telemetry|DeviceStateMapping|DeviceStateMapper" src tests docs\superpowers\audits\2026-05-11-openhab-windows-code-quality-audit.md docs\superpowers\status docs\superpowers\specs\2026-05-04-openhab-windows-sitemap-client-design.md`: run after review feedback. Key result: core mapper and tests exist for battery/charging/locked/session mapping, while `AppSettings` has no telemetry Item configuration fields and no current Wi-Fi/network telemetry path was found.
